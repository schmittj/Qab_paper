...
force every such collision into an explicit finite list of arithmetic
configurations.  The remaining case analysis is computer-assisted but fully
deterministic: bundled verification scripts, which any reader can replay,
eliminate every configuration on the list, the few terminal cases being
settled by modular gcd certificates.
\end{abstract}
...
\item Sections~\ref{sec:padic}--\ref{sec:wronskian} build the arithmetic
reduction: prime separation,
...
\[
\begin{array}{c}
\text{collision dictionary}
\Longrightarrow
\text{structural reductions and local packet sieves}\\[2mm]
\Longrightarrow
\text{upper-range computation}
\Longrightarrow
\text{lower-state envelope}\\[2mm]
\Longrightarrow
\text{unit branch and nonunit branches}
\Longrightarrow
\text{terminal modular certificates}
\Longrightarrow
\text{Theorem~\ref{thm:main}.}
\end{array}
\]
...
proof-assistant certificate.  Table~\ref{tab:code} gives the paper-to-code
correspondence: the generator and the independent re-checker behind each
computational result.

For reference, the following roadmap lists the main mathematical and
computational outputs in the order they are used; it is an index to the proof,
not a substitute for the later arguments.  Each symbol is defined where it
first arises; in brief, \(\{a,b\}\) and \(\{c,d\}\) are the two packages, a
counterexample is a shared irreducible factor whose normalization yields
integers \(0<m<n\) and \(0<p<q\) with \(\gcd(m,n,p,q)=1\) and
\(D=\max\{m,n,p,q\}\); the integers \(r,s\) are the scales of the two
primitive shapes; \(d\) is the degree over \(\Q\) of a common root; \(e\) a
factor degree; \(U\) and \(V\) the leading and constant coefficients of the
primitive minimal polynomial of a common root; and \(M(\cdot)\) the Mahler
measure.
...
\item The nonunit computations eliminate the both-nonunit branch, the
one-nonunit branch with \(D\geq16\,584\), and the residual
one-nonunit branch
\[
...
divisibilities \eqref{eq:17-24}, and their prime divisors force the exact slope
matching \eqref{eq:17-25}--\eqref{eq:17-26}.
\end{enumerate}
...
\begin{equation}
 ...
 \label{eq:2-3}
\end{equation}
The monomial in \eqref{eq:2-3} is harmless because \(Q_{a,b}(0)=b\neq0\).
...
We first show that dividing out the double cyclotomic factor removes every
unit-circle root, so that all collisions of interest occur off the unit
circle.
...
Because the two complementary edges in \eqref{eq:2-4} share their upper endpoint,
Theorem~\ref{thm:nothree} rules out their simultaneous occurrence at an
off-unit-circle root.  With this in hand, we can state the exact equivalence
between package coprimality and the collision graph.
...
edges in \eqref{eq:2-4}, which share their upper endpoint and are forbidden by
...
\begin{proof}
The first equality and \eqref{eq:3-2} give \(L_u=z^uL_v\).  Hence
\[
...
developed in the later sections.
...
\[
 ...
 \leq\frac{2r^m}{1-r^m},
\]
which is \eqref{eq:4-1}.  Since \(k\geq1\), this forces
...
\]
and \eqref{eq:4-2} follows from \(|1-z^m|\geq1-r^m\).
\end{proof}
...
\(m\leq M(r)\), followed by the explicit bound \eqref{eq:4-2}.
...
\begin{proof}
Apply \eqref{eq:3-2} with \((u,v)=(m,a)\):
\[
 (m+a)L_{m+a}=mL_m+az^mL_a.
\]
Since \(L_{m+a}=L_m\), this yields \eqref{eq:6-1}.  Applying \eqref{eq:3-2} with
\((u,v)=(m,j)\), subtracting \((m+j)A\), and using \eqref{eq:6-1}, gives
\[
 (m+j)(L_{m+j}-A)=jz^m(L_j-L_a),
\]
which is \eqref{eq:6-2}.
\end{proof}
...
\eqref{eq:6-3}.  At \(j=a\), both sides are zero directly; here
...
Every configuration \eqref{eq:6-4} satisfies
\begin{equation}
 \frac{s}{m+s}(L_s-L_a)
 =\frac{t}{m+t}(L_t-L_a),
 \label{eq:6-5}
...
\begin{proof}
Apply \eqref{eq:6-2} with \(j=s,t\), then use
\(L_{m+s}=L_{m+t}\).  This gives \eqref{eq:6-5}, and solving it gives
\eqref{eq:6-6}.  The inequalities \(0<\lambda<1\) are equivalent to \(0<s<t\).
...
Clearing denominators in \eqref{eq:6-5} gives a useful sparse algebraic residue of
...
\(H_{m,m+a}(z)=0\) and the quadrinomial \eqref{eq:6-8}, as well as the strict
segment condition \eqref{eq:6-6} and the shifted sign law \eqref{eq:6-3}.
...
\(\arcsin(\rho/R)\).  Apply this to \eqref{eq:7-1}.  The second collision gives
\[
 z^{m+s}\bigl(m+t-(m+s)z^{t-s}\bigr)=t-s,
\]
which proves \eqref{eq:7-3}.  Circular distance to \(2\pi\Z\) is subadditive,
so subtracting the two multiples of \(\theta\) gives \eqref{eq:7-4}.  If
...
The quadrinomial in \eqref{eq:6-8} has a useful decomposition.  Continue to write
...
\begin{proof}
Expanding the right side of \eqref{eq:7-5} gives \eqref{eq:6-8}.  For \(0<u<v\),
\begin{equation}
 \frac{H_{u,v}(x)}{(x-1)^2}
...
\eqref{eq:7-5} are of this form and both scalar coefficients are positive.
...
Dividing \eqref{eq:7-5} therefore gives \eqref{eq:7-6}.
\end{proof}
...
\eqref{eq:7-6} would have to lie on opposite rays.  The nested case \(t<a\)
...
\section{The Schur deformation}
\label{sec:schur}

The trinomial \(H_{m,n}\) extends to a two-variable Schur deformation, whose
\(t=1\) specialization recovers \(H_{m,n}/(x-1)^2\).  We record that
deformation here; the next section expands it to second order in \(t\) to
compare the root branches of two colliding edges.
...
\section{The second jet of the Schur deformation}
\label{sec:secondjet}
...
\[
 ...
 x=e^{\tau/2}y.
\]
Writing $|\lambda|$ for the size of the partition, homogeneity and symmetry
in the first two variables give
\[
 e^{-|\lambda|\tau/2}s_\lambda(1,e^\tau,e^{\tau/2}y)
...
\begin{proof}
Expanding \eqref{eq:9-1} at \(\tau=0\) and using \(H_{m,n}(z)=0\) gives
\begin{equation}
 S_{m,n}(z,\tau)
 =\frac{\tau^2}{24(z-1)^2}
...
\eqref{eq:9-3}--\eqref{eq:9-5} gives \eqref{eq:9-2} with remainder \(O(\tau^4)\).
\end{proof}
...
bracket in \eqref{eq:9-6} vanishes.
For example, if \(m+n=p+q\), higher contact would force \(A=B\), contradicting
the no-three theorem for disjoint edges.  This second-order computation is the
substitute for the failed first-order transversality argument.
...
The rational-root theorem applied to \eqref{eq:10-1} gives
\[
...
\(\ell\nmid b(a+b)r\), the two terms on the right of \eqref{eq:10-2} have
...
\begin{equation}
 ...
 \label{eq:10-5}
\end{equation}
If \(r\geq2\), then \(b\geq2^a>a\), while \(s\geq2\) and \eqref{eq:10-5} give
\(a\geq2^b>b\), a contradiction.  Hence \(r=1\).

Write \(a=s^bA\) with \(\gcd(A,s)=1\).  Dividing \eqref{eq:10-2} by \(s^b\)
gives
\begin{equation}
...
\(a=2\), and \eqref{eq:10-5} gives \(s=2\), \(b=1\), \(A=1\).  Thus the only
primitive root in the open unit disk is \(-1/2\) for \(Q_{2,1}\).  Reciprocity
gives \(-2\) for \(Q_{1,2}\).

Finally, for a nonprimitive pair write \(a=gA\), \(b=gB\).  By \eqref{eq:2-5}, a
...
\(Q_{C,D}(x^h)\) also has square-free reduction.  Equation \eqref{eq:2-5} differs
...
\]
By \eqref{eq:11-3}, both \(u\) and \(v\) belong to the rational \(S\)-unit
...
\((\C^\times)^2\) of rank \(r\) by \(2^{8r+8}\), giving \eqref{eq:12-1}.
...
The next theorem also covers the boundary primitive shapes.  Its principal
...
\begin{equation}
 ...
 \label{eq:13-2}
\end{equation}
In particular, \(\gcd(C_k,b)=1\), so the coefficient in \eqref{eq:13-1} has
...
\]
Putting all terms over \(b^k\) gives \eqref{eq:13-2}.  Every term with \(r\geq1\)
...
orientations in \eqref{eq:13-3} must agree.  We therefore assume that the packages
...
\eqref{eq:4-1}, applied to the edge \(\{c,c+d\}\), gives
\[
...
\(e^y(1-y)-1<0\).  Using \eqref{eq:13-9} in \eqref{eq:13-11}, and then
...
The orientation bookkeeping in the proof is as follows.  If the unordered
packages agree, the two reciprocal orientations are coprime by the no-three
theorem, so a divisibility relation must preserve the selected orientation.
If the packages are distinct, Corollary~\ref{cor:triples} gives disjoint
scaled triples, and the coefficient comparison fixes the leading and constant
roles separately.  The boundary case is reduced by reciprocity before the
diagonal-denominator argument is applied.  Thus no step identifies
\((a,b)\) with \((b,a)\) unless that reciprocal orientation has first been
handled explicitly.
...
\[
 ...
 \leq q'=\frac q{d_2}.
\]
Interchanging the two trinomials gives the other half of \eqref{eq:14-1}.  Their
...
\((x-1)^2\), lies in the gcd, proving \eqref{eq:14-2}.  Here the fibre estimate is
the elementary Lemma~\ref{lem:schinzelfibre}, so the bound relies on no
external result.
\end{proof}
...
root-multiplicity budget is at most \(p-2\), proving \eqref{eq:14-3}.
\end{proof}
...
\[
 A=u^p,\qquad B=v^p
\]
for coprime positive integers \(u,v\).  For \(p=2\), equation \eqref{eq:14-9}
...
\]
a contradiction.  This proves \eqref{eq:14-8}.
\end{proof}
...
Section~\ref{sec:kummer} handles irreducible primitive bases.  For reducible bases, the same power questions must be tracked factor by factor.  The purpose here is to record exactly where endpoint primes can still create internal powers and which congruence signatures survive.
...
\(p-2\) modulo \(p\).  This proves \eqref{eq:15-1} for odd \(p\).
...
even.  This is precisely \eqref{eq:15-1} modulo \(2\).

Thus every \(p\mid N\) divides \(e(e+2)\), proving \eqref{eq:15-2}.  For a
...
\(e(e+2)\leq\nu(\nu-2)\), contradicting \eqref{eq:15-3}--\eqref{eq:15-4}.
...
\end{enumerate}
If \(pe>\mu\), the second alternative holds and hence all applicable
...
\(pe\leq[\Q(w):\Q]\leq\mu\).  The contrapositive gives the second
alternative when \(pe>\mu\).
\end{proof}

\begin{remark}
The former condition \(2e>\mu\) is the special case in which the
...
integers.  Multiplying the primes in their two roles gives \eqref{eq:15-7}.  The
...
\]
which is the final inequality in \eqref{eq:15-8}.  If \(q>k+1\), then
...
valuations gives the first relation in \eqref{eq:15-9}.  Integrality of the
...
\begin{proof}
At a root \(\alpha\) of \(Q_{a,b}\), equation \eqref{eq:16-1} gives
\[
...
\]
This proves \eqref{eq:17-1}, and direct expansion gives \eqref{eq:17-2}.
...
primitive, so Theorem~2.6 applies under \eqref{eq:17-3}.  It gives the displayed
...
root \(z\) by the corresponding power, as in \eqref{eq:2-6}.  The normalized
...
\]
which is the first inequality in \eqref{eq:17-5}.
...
Adding them proves \eqref{eq:17-6}.
\end{proof}
...
remain.  Therefore Cauchy--Schwarz and \eqref{eq:17-8} give
\[
 \deg\overline T
...
\]
Thus \(\Q(P_\alpha)=\Q(\alpha)\), proving \eqref{eq:17-10}.
\end{proof}
...
\(Q_{b,a}\), contradicting the nonzero reciprocal resultant \eqref{eq:16-3}.
...
\[
 \frac{8\log(2\cdot175394638)}{175394638^{1/3}}
   <\log\theta_0.
\]
Thus \eqref{eq:17-14} is impossible for \(D\geq175394638\).  The smaller values
...
\]
Hence \eqref{eq:17-13} and Proposition~\ref{prop:fielddegree} give
...
Modulo \(\ell\), the summand in \eqref{eq:17-15} is constant on such a cluster,
...
\eqref{eq:17-16}.  The argument also covers \(\ell=2\); the merged side still
...
prime.  Multiplication by \(\widetilde h(t)\) gives \eqref{eq:17-17}, and the
constant and \(t^{e-1}\) coefficients give \eqref{eq:17-18}.
\end{proof}
...
unit at those primes.  Thus \eqref{eq:17-19} follows.
...
\[
 ...
 \leq2^ee^eM(h)^{2e-1}.
\]
Taking \(e\)-th roots proves \eqref{eq:17-20}.
\end{proof}
...
\]
formula \eqref{eq:17-20} gives \eqref{eq:17-22}.  In the unit-normalized case no prime
is omitted, so \(\mathcal S=\mathcal R\), giving \eqref{eq:17-23}.
\end{proof}
...
\eqref{eq:17-25}.  The positive-valuation endpoint description gives
\eqref{eq:17-26} in exactly the same way when \(\ell\mid v\).
\end{proof}
...
Table~\ref{tab:code} lists, for each computer-assisted result in
Sections~\ref{sec:computation}--\ref{sec:nonunitcomputation}, the generator
that produces its data and the independent script that re-checks it.

\begin{table}[ht]
\centering\footnotesize
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.27\textwidth}>{\raggedright\arraybackslash}p{0.64\textwidth}@{}}
\hline
Result & Generator \(\to\) independent verifier\\
\hline
Upper range, Thm~\ref{thm:upperelimination}
 & \path{archive/Qab9_upper_bundle} pipeline \(\to\) \path{verify_upper_safe.py}; constants \path{certify_constants.py}, \path{certify_thresholds.sage}\\
Unit branch, Thm~\ref{thm:unitelimination}
 & \path{enumerate_unit_*.cpp}, \path{pair_unit_*.cpp} \(\to\)
 \path{verify_unit_branch_manifest.py}, \path{verify_modular_gcd_certificates.py},
 \path{verify_irreducibility_certificates_direct.py}\\
Both-nonunit, Thm~\ref{thm:twononunitelimination}
 & \path{enumerate_two_nonunit_packages.cpp}, \path{pair_two_nonunit.cpp} \(\to\) \path{verify_two_nonunit.py}\\
One-nonunit \(D\ge16\,584\), Thm~\ref{thm:onenonunitlargeelimination}
 & \path{enumerate_one_nonunit_packages.cpp}, \path{pair_one_nonunit.cpp} \(\to\) \path{verify_one_nonunit.py}\\
Residual one-nonunit, Thm~\ref{thm:onenonunitresidualelimination}
 & \path{enumerate_one_nonunit_residual_packages.cpp}, \path{pair_one_nonunit_residual.cpp} \(\to\) \path{verify_one_nonunit_residual.py}\\
Analytic constants
 & \path{certify_qab12_constants.py}\\
\hline
\end{tabular}
\caption{Paper-to-code correspondence for the computer-assisted steps.  Scripts
live in \protect\path{code/qab12} and data in \protect\path{data/qab12}, except
the upper-range pipeline in \protect\path{archive/Qab9_upper_bundle};
reproduction commands are in Section~\ref{sec:computation}.}
\label{tab:code}
\end{table}
...
right-hand side of \eqref{eq:18-1} gives the exact rational lower bound
\[
 \theta_0^{70}>
...
\begin{proof}
The height estimate \eqref{eq:17-13} and the Mahler gap \eqref{eq:17-11} give
\[
 d\geq\frac{D\log\theta_0}{2\log(2D)}.
\]
Lemma~\ref{lem:plasticpower} gives
\(70\log\theta_0>\log(2D_1)\geq\log(2D)\), proving \eqref{eq:18-2}.
...
\(s\mid e_1\) and \(r\mid e_2\), giving \eqref{eq:18-4}.
\end{proof}
...
\(\mathcal R=\operatorname{rad}(AB(A+B))\), giving \eqref{eq:18-5}.
\end{proof}
...
\begin{proof}
Let \(f_\beta\) be the minimal polynomial of \(\beta\).  From \eqref{eq:18-4},
\[
 \log M(f_\beta)
 =e_1\,\mathrm h(\beta)
 =e_1r\,\mathrm h(\alpha)
...
using \(e_1<4D^{2/3}/r\) together with \eqref{eq:17-13}, we obtain
\[
 \begin{aligned}
...
enough to bound \(C(D_1)\).  The exact rational certificate listed in
Table~\ref{tab:code} uses
\[
...
the residual-split merged side.  This proves \eqref{eq:18-7} in every role.
\end{proof}
...
\(\alpha\).  Bezout's theorem now gives \eqref{eq:18-9}.
\end{proof}
...
opposite scale;
\item the two support caps \eqref{eq:18-6}, the range \(D_0\leq D\leq D_1\),
...
Theorem~\ref{thm:explicitbound} proves \eqref{eq:18-10}.
\end{proof}
...
The upper computation of Section~\ref{sec:computation} gives
\[
 D:=\max\{m,n,p,q\}\leq6\,816\,241.
\]
This section records the lower-range identities in a form suitable for exact
enumeration, and treats in full the exceptional fourth-power case of Capelli's
criterion.  Write the two normalized packages as
\[
 \{ra,rb\},\qquad \{sc,se\},
...
\[
 F=\Q(\alpha),\quad K=\Q(\alpha^r),\quad K'=\Q(\alpha^s),
\]
\[
 d=[F:\Q],\quad e_1=[K:\Q],\quad e_2=[K':\Q],
 \quad t=[F:K],\quad u=[F:K'].
\]
...
from \(\alpha^r,\alpha^s\), so \(F=KK'\).  Finally
\([KK':\Q]\leq[K:\Q][K':\Q]\).
\end{proof}
...
For equal \emph{selected ordered} orientations, \eqref{eq:19-2} gives \(d\leq2rs\).
...
\eqref{eq:19-6}--\eqref{eq:19-7} and Proposition~\ref{prop:reducedcorrespondence} give
\[
...
incompatible for every \(D\geq16\,584\); the comparison is certified by the
analytic-constants verifier listed in Table~\ref{tab:code}.
\end{proof}
...
good, so \(e_1\leq p-2\leq137\), and \eqref{eq:19-11} follows from
...
\begin{gather}
 ...
 \qquad 140d>D,
 \qquad d^3<64D^2,
 \qquad d\leq\min\{a+b,c+e\}-2,\label{eq:19-12}
\end{gather}
...
In the verification bundle these fields are represented by typed CSV rows.  The
separate verifiers listed in Table~\ref{tab:code} recompute the global integer
conditions from those rows, and the small-box generators supply coverage
regressions for the enumeration logic.
...
\]
This is the same envelope \(C(D)\) as in Corollary~\ref{cor:unitsupportcap},
but restricted to \(D\geq50\,000\); the analytic-constants verifier listed in
Table~\ref{tab:code} prints the exact rational certificate
\[
 C(50\,000)<1\,612\,000.
\]
The generator keeps \(63\,980\) primitive shapes.  The scale-pair scan
considers \(334\,910\,032\) indexed shape pairs and applies
...
enumeration.  It produces \(24\,301\) shape--degree package records.  The
exact pair scan reduces \(734\,084\) raw state pairs to four terminal state
pairs and three terminal package pairs.  Their twelve selected orientation
pairs are certified by the same modular-gcd
...
recomputers are listed in Table~\ref{tab:code}.  The bundled checksums and
the unit-branch manifest provide the row counts, terminal counts, and
...
\eqref{eq:19-8}, \eqref{eq:19-10}, and the slope identities.
\end{enumerate}
\end{corollary}
...
This section presents the nonunit computation.  It is kept
separate from the unit proof because the nonunit package states retain exact
...
state-pair file is empty, as confirmed by the independent verifier listed in
Table~\ref{tab:code}.
\end{proof}
...
\]
gives the sharp coefficient-specific cap recorded by the
analytic-constants verifier listed in Table~\ref{tab:code}: the largest cap
is \(6\,816\,241\) for \(U=2\), and the cap for \(U=26\) is \(16\,743\).
...
independent verifier listed in Table~\ref{tab:code} recomputes the row
conditions, confirms the per-\(U\) pair counts, and verifies that every
...
orientations, the recorded residual certificates give the prime \(p=1009\)
and modular gcd degree \(2\).  The independent residual verifier listed in
Table~\ref{tab:code} recomputes every package row, reruns the pair sieve,
compares the terminal CSV files exactly, and recomputes all eight modular
gcds over \(\mathbf F_{1009}\).  Degree \(2\) is accounted for by the forced
double root at \(x=1\), so no noncyclotomic common factor remains in any
terminal orientation.
\end{proof}

\subsection*{Assurance status of the verification bundle}

The bundled verification artifacts eliminate the upper range, the full
unit branch, the both-nonunit branch, the one-nonunit branch with
\(D\geq16\,584\), and the residual one-nonunit branch with
...
common factor.  The nonunit artifacts eliminate both endpoint coefficients
being nonunit and also eliminate the whole one-nonunit branch above
\(16\,583\).

The residual one-nonunit computation then eliminates the remaining
...
\section{Computer-Assisted Proof Contract}
\label{app:proofcontract}

This appendix records the verification contract behind the finite computations.
The scripts are deterministic and use exact integer or finite-field
arithmetic.  A generator is trusted only for exhaustive coverage of the rows it
emits; the corresponding verifier checks the row schema, recomputes all stated
integer predicates or terminal certificates listed below, and compares bundled
hashes or manifests.

\begin{description}[leftmargin=1.8em,style=nextline]
\item[Upper range.]
The archived Qab9 pipeline enumerates all rows in the certified range.
\path{verify_upper_safe.py} checks 237,418 shapes, 761 prefinal pairs, and no
residual candidates.  The remaining trust is archived coverage.

\item[One-nonunit \(D\ge16\,584\).]
The per-\(U\) package generators enumerate all admissible rows for
\(U=2,\ldots,26\).  \path{verify_one_nonunit.py} checks schemas, row
predicates, counts, and empty terminal CSVs.  The remaining trust is generator
coverage.

\item[Residual one-nonunit.]
The residual generator enumerates the corrected \(d\le2601\) range.
\path{verify_one_nonunit_residual.py} reruns the pair sieve, compares terminal
CSVs, and recomputes eight gcd certificates over \(\mathbf F_{1009}\).  The
remaining trust is generator coverage.

\item[Unit full-Kummer branches.]
The unit generators enumerate the large and small unit package rows.
\path{verify_unit_branch_manifest.py} checks stage counts, hashes, terminal
pair counts, and certificate coverage; \path{verify_modular_gcd_certificates.py}
recomputes the FLINT gcds.  The remaining trust is unit enumeration coverage.

\item[Unit deficient-scale branch.]
The defect generators enumerate the Kummer-defect rows.  The unit manifest
checks the row and terminal counts, and
\path{verify_irreducibility_certificates_direct.py} recomputes the 46
irreducibility certificates.  The remaining trust is defect enumeration
coverage.
\end{description}
...
U,d,sigma1,...,sigma5,r,t,e1,a,b,n,s,u,e2,c,f,m,D,correspondence_bound,role_bound
...
d,r,t,e1,a,b,n,defect1,s,u,e2,c,f,m,defect2,D,role_bound,correspondence_bound
\end{lstlisting}

The public non-mutating verification command is \texttt{make verify-light}.
The FLINT-dependent arithmetic replay is \texttt{make verify-unit}.  The
residual manifest is checked with \texttt{--check-manifest}; it is regenerated
only with the explicit \texttt{--write-manifest} mode.

\section{Local Packet Valuation Table}
\label{app:localpackets}

The local packet arguments use the following cases, all with the conventions
translated from Borisov's notation in Sections~\ref{sec:padic} and
\ref{sec:kummer}.  For a prime \(p\) dividing exactly one role of a primitive
triple, Borisov's endpoint lemmas give the packet center and slope denominator.
At a total prime \(p\mid A+B\), every unit root lies in a nontrivial
root-of-unity packet, and the packet contribution is divisible by \(p\) except
for the center-\(1\) residue already recorded in the role congruence.  At a
leading endpoint prime \(p\mid A\), Lemma~2.10 of \cite{Borisov1998} gives the
nonunit block of length \(B\) and the unit packet counted by Lemma~2.11; the
constant endpoint is reciprocal.  If \(p=2\), the first two formal layers must
be kept merged, producing the residual split used in
Lemma~\ref{lem:oreresidual}.  This is the only place where Capelli's
exceptional \(-4K^4\) branch can survive, and
Lemma~\ref{lem:exceptionalfourth} shows that a survivor is forced into the
nonunit endpoint block with \(e\leq B\) and the stated norm divisibility.

\section*{Use of AI tools}

This project used AI tools in discovery, editing, and implementation.  The
initial proof strategy was found through multiple rounds of prompting to
ChatGPT 5.5 Pro.  Claude Code (Opus 4.8 max) was used for manuscript-level
revision and audit-oriented cleanup.  The Codex CLI tool running gpt-5.5 xhigh
edited the manuscript, implemented and revised verification scripts, finalized
the computer-verification artifacts, and ran the reproducibility checks.  The
mathematical claims in the paper are supported by the written arguments and by
the deterministic proof artifacts described in Appendix~\ref{app:proofcontract};
the AI transcripts themselves are not used as proof evidence.
