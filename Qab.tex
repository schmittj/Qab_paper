...
Unit branch, Thm~\ref{thm:unitelimination}
 & \path{enumerate_unit_*.cpp}, \path{pair_unit_*.cpp} \(\to\) \path{verify_modular_gcd_certificates.py}, \path{verify_irreducibility_certificates_direct.py}\\
...
branch-specific verifier outputs provide the row counts and SHA-256 bindings
used for replay.
\end{proof}
...
The manuscript was refined with Claude Code (Opus 4.8 max) and edited and
revised with the Codex CLI tool running gpt-5.5 xhigh; Codex also finalized
the computer verification part.  A fuller account of the AI-assisted
discovery, editing, and verification process will be added in a later
revision.
