...
The final residual computation uses the corrected safe degree envelope for
the small one-nonunit endpoint range
\[
 D\leq16\,583,\qquad |V|=1,\qquad 2\leq U\leq D,
\]
where the degree is at most \(2601\).  It enumerates \(233\,278\) conservative
package states, reduces them to four terminal state pairs and two package
pairs, and eliminates all terminal orientations by modular gcd certificates.
Together with the earlier upper, unit, and nonunit computations, this
eliminates every normalized reciprocal-package counterexample.
\end{abstract}
...
finite box reached by the mathematical reductions.  The present bundle is
therefore a reviewable computer-assisted proof package for the
reciprocal-package coprimality statement.
