...
\title{Explicit Boundedness and Nonunit Branch Reductions\\
...
\(D\leq6\,816\,241\).  This revision keeps the repaired Capelli
\(-4K^4\) treatment and the complete unit-coefficient computation, and adds
new production artifacts for the genuinely nonunit branches.

The accompanying deterministic computations now eliminate four large pieces
of the lower box.  First, the unit-coefficient branch is exhausted: full
Kummer degree above and below \(50\,000\), and every deficient-scale unit case,
are excluded by modular gcd and modular subset-degree certificates.  Second,
the both-coefficients-nonunit branch is confined to \(D\leq16\,583\) and then
has no compatible pair of one-package states.  Third, after reciprocal
normalization the one-nonunit branch with \(D\geq16\,584\) has
\(2\leq U\leq26\); a conservative all-shapes enumeration over these
coefficients produces \(608\,490\) package states and \(137\,530\) raw
same-coefficient state pairs, all eliminated before terminal polynomial
construction.

The mathematical inputs are the exact extraction identities
\(d=te_1=ue_2\), the reduced power-correspondence bound
\[
 d\leq\frac{rs(ae+bc)}{\gcd(rb,se)\gcd(ra,sc)},
\]
norm transfer, endpoint divisibilities, slope matching, local packet
occupancies, and the repaired exceptional fourth-power analysis.  The new
code includes independent row/count verifiers for the nonunit catalogues and
recomputing verifiers for the unit terminal certificates.

The package is close to a complete computer-assisted proof, but it still does
not claim the full conjecture.  The remaining production gap is now the small
one-nonunit endpoint range
\[
 D\leq16\,583,\qquad |V|=1,\qquad 2\leq U\leq D,
\]
where the degree is at most \(224\).  The bundle records this boundary
explicitly so that the partial proof artifacts and the final small-range
enumerator can be independently audited.
\end{abstract}
...
\item The new nonunit computations eliminate the both-nonunit branch and the
one-nonunit branch with \(D\geq16\,584\).  Consequently every remaining
candidate, if any, lies in the small one-nonunit range
\[
 D\leq16\,583,\qquad |V|=1,
 \qquad 2\leq U\leq D,
\]
after reciprocal normalization.
...
primitive shapes.  It does not settle the full conjecture by itself: the
residual reducible--reducible families still require finite arithmetic
elimination.  The residual-polynomial splitting theorem removes the former
local \(2\)-adic exception; the height--degree argument and the computation in
Section~\ref{sec:computation} eliminate the complete upper unit-normalized
range; Section~\ref{sec:unitlowercomputation} eliminates all unit factors in
the lower box; and Section~\ref{sec:nonunitcomputation} eliminates all
nonunit branches except the explicitly isolated small one-nonunit range
\(D\leq16\,583\).
...
recomputers are in \path{code/qab12} and \path{data/qab12}.  The
...
\begin{corollary}[Current residual envelope]
...
most \(14\), the degree satisfies \(d\leq224\), and every deficient endpoint
prime lies in \(\{2,3,5,7\}\).
\end{corollary}
...
unit branch, the both-nonunit branch, and the one-nonunit branch with
\(D\geq16\,584\).  The package therefore moves the project very close to a
complete computer-assisted proof, but it does not yet present the final
small-range production enumeration of Corollary~\ref{cor:q12residualenvelope}.
The required next step is a generator allowing \(2\leq U\leq16\,583\) and
\(d\leq224\), followed by the same independent coverage and terminal
certificate checks used above.
...
Consequently the only unresolved production computation is the small
one-nonunit range
\[
 D\leq16\,583,
 \qquad |V|=1,
 ...
 \qquad d\leq224.
\]
This is much smaller than the original finite box and should be suitable for
a direct state enumeration plus modular certificate pass.  The present bundle
is therefore a reviewable Qab12 progress package, not yet a claim that the
full conjecture has been proved.
