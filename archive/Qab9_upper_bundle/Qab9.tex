...
\title{Residual-Polynomial Splitting, Explicit Boundedness,\\
and an Exact Upper-Range Elimination for the Polynomials \(Q_{a,b}\)}
\author{}
\date{25 June 2026}
...
unordered pairs are pairwise coprime over \(\Q\).  The full four-index
conjecture remains open, but the combination of the mathematical reductions
and an exact integer computation now proves that every normalized
counterexample satisfies
\[
 \boxed{\max\{m,n,p,q\}\leq6\,816\,241}.
\]

The local part of the note resolves the merged \(2\)-adic slope \(1/2\) by
its separable quadratic residual polynomial, obtains exact odd-prime factor
signatures, and proves self-power rigidity for every irreducible primitive
base.  Consequently every lift of every irreducible primitive unequal shape
is globally isolated, and Borisov's quantitative estimate gives
\[
 I(X)=O\!\left(X^{20/11}\log X\right)
\]
for the number of vertices of total at most \(X\) not yet proved globally
isolated.

For normalized counterexamples, an elementary height estimate, a
Minkowski-short rank-two subtorus of degree \(<4D^{2/3}\), rank-two
torus-intersection rigidity, and Smyth's nonreciprocal Mahler-measure gap
first give the explicit analytic bound
\[
 D\leq175\,394\,637.
\]
Above \(6\,816\,241\), the common minimal polynomial is monic with constant
coefficient \(\pm1\).  In this unit range a further exact argument gives
\(d>D/140\), forces both package scales to be at most \(139\), and proves a
full Kummer degree identity \(d=rsk\).  The one-package moment--Hankel
argument then yields
\[
 r\operatorname{rad}(AB(A+B))<4\,398\,937
\]
and its analogue for the second package.  Full-radical degree congruences and
role-binomial degree bounds reduce the search to an exact enumeration of
\(237\,418\) primitive shapes and then \(761\) candidate package pairs.  All
\(761\) have the same primitive shape.  A power-correspondence intersection
in \(\mathbb P^1\times\mathbb P^1\) bounds a common field degree by \(2rs\),
contradicting \(d>D/140\) in every case.  The computation uses only integer
and rational arithmetic; the source, data, independent row verifier, and
small-box regression tests accompany this revision.

The remaining range is nonunit and substantially smaller.  Its exceptional
endpoint-power branches involve only the primes \(2,3,5,7\); organizing and
certifying that lower-range computation is the outstanding task.  Exact
factorization through total \(60\) continues to certify all \(870\) tested
packages globally.
\end{abstract}
...
\section{The residual proof requirement}
\label{sec:residual}

The full conjecture is not yet proved.  The self-power theorem removes every
collision having an irreducible primitive base, so the unresolved class is
exactly the following.

\begin{theorem}[Exact residual class]\label{thm:residualclass}
Every counterexample to reciprocal-package coprimality has two reducible
primitive bases.
\end{theorem}

\begin{proof}
Write the packages as \(\{rA,rB\}\) and \(\{sC,sD\}\), with
\(\gcd(A,B)=\gcd(C,D)=1\).  If either primitive polynomial were
irreducible, Theorem~\ref{thm:allliftsisolated} would make every one of its
lifts globally isolated.
\end{proof}

After replacing a common root by its inverse if necessary, the analytic form
of a surviving counterexample may be written
\begin{equation}
 L_m(z)=L_{m+a}(z),\qquad
 L_{m+s}(z)=L_{m+t}(z),\qquad 0<|z|<1.
 \tag{19.1}
\end{equation}
It must satisfy all of the following:
\begin{enumerate}[label=(\roman*)]
\item the six additive-triple entries are pairwise distinct and, after
simultaneous normalization, the gcd of the four endpoints is one;
\item the sign, tail, strict-chord, block-centroid, phase, positive-cone, and
second-jet identities of the earlier sections;
\item the additive-triple prime-support constraints and local finiteness from
Sections~\ref{sec:padic}--\ref{sec:sunit};
\item the radical role signatures of Proposition~\ref{prop:rolesignature}
and the exact factor partitions of Corollary~\ref{cor:partitionsignature};
\item the prime-by-prime Kummer alternatives of Proposition
\ref{prop:largetrigger}, including the endpoint divisibilities of
Theorem~\ref{thm:endpointpower} and the large-prime matching rule of
Proposition~\ref{prop:largerole};
\item the square divisor in the nonzero sparse Wronskian of Proposition
\ref{prop:wronskian};
\item the explicit endpoint bound of Theorem~\ref{thm:explicitbound},
the exact unit-range elimination of Theorem~\ref{thm:upperelimination}, and
the moment, support, endpoint-divisibility, and slope-matching sieves of
Corollary~\ref{cor:supportbound} and Proposition~\ref{prop:endpointarith}.
\end{enumerate}

\subsection{The normalized algebraic core}

Write the two packages as
\[
 \{rA,rB\},\qquad \{sC,sD\},
 \qquad \gcd(A,B)=\gcd(C,D)=1,
\]
and put \(g=\gcd(r,s)\), \(r_0=r/g\), \(s_0=s/g\).  If \(\alpha\) is
a common root in selected orientations and \(w=\alpha^g\), then the minimal
polynomial \(k_w\) is noncyclotomic and divides
\[
 Q_{A,B}(x^{r_0})
 \quad\text{and}\quad
 Q_{C,D}(x^{s_0}).
...
Therefore
\begin{equation}
 \deg k_w\leq\min(A+B,C+D)-2.
 \tag{19.2}
\end{equation}
If \(\beta=w^{r_0}\) belongs to a degree-\(e\) irreducible factor of
\(Q_{A,B}\), then \(\deg k_w\) is a positive multiple of \(e\).  For
each prime \(p\mid r_0\), either \(pe\) fits below the allowance in
\((19.2)\), or \(\beta\) is an internal \(p\)-th power.  Total primes are
now excluded for every \(p\), including the merged \(2\)-adic case.
Good primes satisfy \(e\leq p-2\).  At a bad endpoint, the only remaining
local possibility is that every conjugate of the factor lies in the nonunit
block, with
\[
 e\leq A,\qquad pA\mid e\kappa
\]
or its reciprocal analogue.  In particular, the endpoint exponent must be
at least \(p\).

Theorem~\ref{thm:upperelimination} now lowers the absolute endpoint to
\(6\,816\,241\).  Proposition~\ref{prop:uniformdegree} still gives
\(r,s\leq139\) throughout this lower range.  The obstruction to repeating
the upper computation verbatim is precisely the possible nonunit leading or
constant coefficient: full Kummer degree and the one-package unit support
bound can fail at an endpoint prime.

There is nevertheless a finite exceptional-prime reduction.  If
\(p^\kappa\) exactly divides an endpoint and an internal \(p\)-th power
occurs, Theorem~\ref{thm:endpointpower} forces \(\kappa\geq p\).  Hence
\[
 p^p\leq6\,816\,241,
\]
so
\[
 p\in\{2,3,5,7\}.
\]
The lower-range pipeline should therefore branch only these four endpoint
primes into unit and nonunit packets, use the exact norm and slope conditions
inside each branch, and pass the remaining rows to modular gcd or Wronskian
certificates.
...
The most focused remaining routes are:
\begin{enumerate}[label=(\arabic*)]
\item complete an exact branch classifier for the nonunit endpoint-power
roles at \(p\in\{2,3,5,7\}\), retaining the tiny scale bound
\(r,s\leq139\);
\item combine the exact factor partitions, endpoint norm divisibilities,
and slope matching to collapse those branches before polynomial
construction;
\item use modular gcd or sparse-Wronskian square-freeness certificates only
on the residual rows, with a separate certificate proving coverage of every
branch;
\item in parallel, seek a theoretical improvement of the height or toric
constant, since any lower endpoint directly reduces the exceptional
endpoint-power catalogue.
\end{enumerate}

\section*{Conclusion}

The full four-index conjecture remains open.  After replacing the former
partial coefficient argument at the center \(1\) by Borisov's complete
root-packet statements in Lemmas~2.9--2.11, the principal theorem chain is
proof-complete: the elementary fibre lemma, expanded Frobenius argument,
self-power rigidity, Kummer reduction, global isolation of every lift of an
irreducible primitive shape, and the density estimate
\[
 I(X)=O\!\left(X^{20/11}\log X\right)
\]
stand after the detailed audit corrections.

The residual reducible--reducible class is now controlled at two levels.  The
local theory resolves the merged \(2\)-adic packet, supplies exact factor
partitions and endpoint-power restrictions, and gives the moment,
discriminant, support, endpoint-divisibility, and slope-matching sieves.  The
height--torus--Mahler argument places every normalized counterexample in the
explicit box \(D\leq175\,394\,637\).

The new upper-range argument then turns the unit part of that box into a
small exact calculation.  The plastic-number identity yields \(d>D/140\)
and \(r,s\leq139\); Capelli and the local power sieves give
\(d=rsk\); the one-package support estimate gives the integer radical cap;
and role-binomial and full-radical congruences leave \(761\) rows.  Every row
has the same primitive shape, while the power-correspondence curves in
\(\mathbb P^1\times\mathbb P^1\) give \(d\leq2rs\), eliminating all of
them.  Thus any normalized counterexample must satisfy
\[
 \boxed{D\leq6\,816\,241}.
\]

The remaining range is genuinely different rather than merely larger: the
common factor may be nonunit, so endpoint internal powers can interrupt the
full Kummer identity.  Those exceptional branches involve only
\(p=2,3,5,7\).  The supplied code therefore constitutes a complete exact
pipeline for the upper unit range and a modular backend for future lower
survivors, but it is not yet a complete proof of the conjecture.  The next
finite task is a coverage-certified classification of the four exceptional
endpoint-prime branches.
