...
\title{Explicit Boundedness and Complete Unit-Branch Elimination\\
for the Polynomials \(Q_{a,b}\)}
\author{}
\date{25 June 2026}
...
\[
 ...
 {(x^{\gcd(a,b)}-1)^2},
\]
we study the conjecture that reciprocal packages attached to distinct
unordered pairs are pairwise coprime over \(\Q\).  The previously audited
argument reduces every normalized counterexample to
\(D\leq6\,816\,241\).  This revision repairs Capelli's exceptional
\(-4K^4\) branch, strengthens the coefficient cutoffs, and carries out an
exact computation for the complete unit-coefficient part of that box.

Without assuming full Kummer degree, extraction degrees satisfy
\(d=te_1=ue_2\) and \(d\leq e_1e_2\).  A reduced power correspondence gives
\[
 d\leq\frac{rs(ae+bc)}{\gcd(rb,se)\gcd(ra,sc)},
\]
while norm transfer supplies exact endpoint divisibilities, slope matching,
and packet occupancies.  If both endpoint coefficients of the common minimal
polynomial are nonunits, then \(D\leq16\,583\).  Above that point at most
one coefficient is nonunit and its absolute value is at most \(26\), improving
to \(12\) for \(D\geq50\,000\).

The accompanying deterministic computation exhausts three unit branches:
full Kummer degree above and below \(50\,000\), and every deficient-scale
case.  Terminal package pairs are excluded by independently recomputed
finite-field gcd certificates.  Forty-one primitive shapes in the defect
branch are certified irreducible by intersections of modular factor-degree
subset sums.  Hence no counterexample has unit leading and constant
coefficients.

The full conjecture is not yet claimed.  What remains is a finite genuinely
nonunit endpoint computation: both-nonunit states lie below \(16\,584\); in
the upper part there is one coefficient in a set of at most twenty-five
values; and deficient endpoint primes are only \(2,3,5,7\).  The bundle
contains a strict state schema, reference verifier, job architecture, all
unit proof artifacts, and independent terminal-certificate checkers.
\end{abstract}
...
nonunit.  If both endpoint coefficients are nonunits then
\(D\leq16\,583\); for \(D\geq16\,584\) there is at most one nonunit
coefficient and it is at most \(26\).
...
\texttt{code/qab11/certify\_qab11\_constants.py}.
\end{proof}
...
The strict JSON schema in
\texttt{code/lower/endpoint\_state.schema.json} represents these fields;
unknown fields are rejected.  The separate reference verifier recomputes the
global integer conditions, and a literal small-box generator supplies a
coverage regression.
...
The calculation has three exhaustive branches.
...
\]
Writing \(x=D^{1/3}\), the logarithmic derivative of the envelope has
numerator
\(2x+48-16\log(2x^3)\), whose derivative is positive for \(x>24\);
therefore the maximum is attained at an endpoint.  Arb certifies both
endpoint comparisons.  The generator finds \(59\,063\) reducible-candidate
primitive shapes.  The scale-pair scan considers \(101\,752\,371\) indexed
shape pairs and leaves \(31\,818\) rows after the radical congruences.  The
rigorous height filter, exact role-binomial test, modular factor-degree
feasibility, and correspondence bound leave \(753\) exact degree rows.
Of these, \(752\) are eliminated by the same-unordered-shape bound
\(d\leq2rs\).  The final package pair is
\[
 (r,a,b)=(14,1,2400),\qquad(s,c,e)=(69,1,2400),
 \qquad d=1932.
\]
For each of its four selected orientation pairs, reduction modulo \(1009\)
has good degree and collision-trinomial gcd of degree exactly two.  Thus the
only common factor modulo \(1009\) is the universal \((x-1)^2\), excluding
a noncyclotomic common factor over \(\Q\).
...
enumeration.  It produces \(9491\) shape--degree records.  The exact pair
scan reduces \(7\,366\,972\) raw pairs to three degree rows representing two
package pairs.  Their eight orientations are certified by modular gcds at
\(997\) or \(1009\), again with gcd degree two.
...
at most \(137\).  Enumerating the possible extraction cores first and then
the smooth additive triples gives \(21\,321\) shape--degree records.  The
pair scan reduces \(105\,874\,335\) raw shape pairs to \(20\,774\) integer
rows and \(19\,346\) rows after exact modular factor-degree feasibility,
representing \(8775\) package pairs.  Only \(41\) primitive shapes occur.

Lemma~\ref{lem:subsetdegreecertificate} certifies every one of these
\(41\) shapes as irreducible.  Forty certificates use small-prime FLINT
factorizations.  For the largest shape \((1,6655)\), the primes
\[
 2,3,7,13,17
\]
reduce the possible rational factor-degree counts successively to
\[
 6655,\ 382,\ 4,\ 4,\ 2;
\]
the final set is \(\{0,6654\}\).  The exact factor-degree lists are stored
in the certificate file and recomputed by a separate verifier.  The global
isolation theorem for irreducible primitive bases now excludes every row in
this branch.

The branch generators, output tables, certificates, and independent
recomputers are in \texttt{code/qab11} and \texttt{data/qab11}.  The
machine-readable branch manifest binds their row counts and SHA-256 hashes.
\end{proof}
...
\hline
full Kummer, \(D\geq50\,000\) & \(101\,752\,371\) pairs & \(1\) pair & \(4\) gcd certificates\\
full Kummer, \(D<50\,000\) & \(7\,366\,972\) pairs & \(2\) pairs & \(8\) gcd certificates\\
Kummer defect & \(105\,874\,335\) pairs & \(41\) shapes & \(41\) irreducibility certificates\\
\hline
\end{tabular}
...
\subsection{Assurance boundary and the next production run}

The unit branch is now represented by deterministic outputs and terminal
certificates.  The remaining computation is strictly the nonunit envelope of
Corollary~\ref{cor:remainingnonunit}.  The bundle supplies a strict state
schema, an integer reference verifier, a tiny-domain oracle, and a job
catalogue for a longer nonunit run.  It does not yet contain a complete
production enumeration of those nonunit states.  Thus Theorem
\ref{thm:unitelimination} is a computer-assisted theorem, while the full
package-coprimality conjecture remains conditional on the final nonunit
enumeration.

The required proof-grade completion is now sharply delimited: generate every
one-nonunit and two-nonunit state, compare a canonical hash with a structurally
independent coverage implementation, and attach either a modular-gcd,
Wronskian, or irreducible-base certificate to every terminal state.  The
current observed unit workload is far below a 25-core, seven-day budget; the
nonunit packet-occupancy and slope equations are intended to keep the final
workload in the same regime.
...
\[
 D\leq6\,816\,241.
\]
The exceptional \(-4K^4\) branch is now handled separately, so the defect
localization no longer invokes an ordinary internal-power theorem outside its
hypotheses.  The general correspondence, norm, endpoint, and packet-occupancy
identities then support an exact computation eliminating the entire
unit-coefficient branch, including every Kummer defect.

Consequently any counterexample is genuinely nonunit.  Two nonunit endpoint
coefficients force \(D\leq16\,583\); above that value there is a single
nonunit coefficient at most \(26\), and above \(50\,000\) at most \(12\).
The remaining exceptional endpoint primes are only \(2,3,5,7\).  This is a
substantial further reduction, but the final nonunit production enumeration
and its independent coverage audit remain outstanding.  The bundle therefore
presents a new computer-assisted theorem and a tightly specified final
calculation, not yet a claim that the complete conjecture has been proved.
