...
\title{Explicit Boundedness, Exact Upper Elimination,\\
and Lower-Range Endpoint States for the Polynomials \(Q_{a,b}\)}
\author{}
\date{25 June 2026}
...
\[
 Q_{a,b}(x)=
 \frac{a x^{a+b}-(a+b)x^a+b}
 {(x^{\gcd(a,b)}-1)^2}
 \qquad(a,b\geq1),
\]
we study the conjecture that reciprocal factor packages attached to distinct
unordered pairs are pairwise coprime over \(\Q\).  The preceding revision
proved, by an exact integer computation, that every normalized counterexample
satisfies
\[
 \max\{m,n,p,q\}\leq6\,816\,241.
\]
The present revision develops the lower-range algebra needed to turn that
remaining box into an auditable finite-state computation.

The principal new geometric input is a reduced power-correspondence bound.
If selected primitive orientations have shapes \((a,b)\), \((c,e)\) and
coprime scales \(r,s\), then every common irreducible factor of degree \(d\)
satisfies
\[
 d\leq
 \frac{rs(ae+bc)}
 {\gcd(rb,se)\gcd(ra,sc)}.
\]
This contains the former same-shape estimate \(d\leq2rs\).  We retain the
actual extraction degrees
\[
 t=[\Q(\alpha):\Q(\alpha^r)],\qquad
 u=[\Q(\alpha):\Q(\alpha^s)]
\]
instead of assuming full Kummer degree.  Thus, with \(e_1,e_2\) the two
primitive factor degrees,
\[
 d=te_1=ue_2,\qquad d\leq e_1e_2.
\]
Scaled norm identities give exact endpoint divisibilities and slope matching.
If both endpoint coefficients of the common minimal polynomial are nonunits,
these estimates force \(D\leq37\,799\).  Above \(50\,000\), at most one
coefficient is nonunit and its absolute value is at most \(12\).

A deficient scale prime is also localized without assuming full Kummer
degree.  In the unit case it is a good prime and forces a small primitive
factor degree.  In the nonunit case, the only branch not covered by that
small-degree alternative is an endpoint packet at a prime in
\(\{2,3,5,7\}\), with exact norm, slope, and packet constraints.  These
conditions define a finite certificate object for every lower-range branch.

The accompanying bundle retains the independently audited upper computation
and adds exact constant certificates, a lower-state schema, reference
checkers, modular-gcd certificate tooling, and an explicit proof-status
boundary.  The new reductions are proved in the text, but the production
endpoint-state enumeration has not yet received an independent coverage
audit.  Accordingly this revision is a candidate route to a complete
computer-assisted proof, not yet a claim that the full conjecture has been
proved.
\end{abstract}

\tableofcontents

\section{Statement, strategic choice, and principal results}

The three sufficient targets in the motivating problem are:
\begin{enumerate}[label=(\alph*)]
\item every unordered unequal pair has a globally private irreducible factor;
\item reciprocal packages of distinct unordered pairs are pairwise coprime;
\item every \(Q_{a,b}\), \(a\neq b\), is irreducible over \(\Q\).
\end{enumerate}
The primitive subcase of (c) is Borisov's irreducibility problem for the
\(abc\)-polynomials \cite{Borisov1998}; the general case additionally contains the composition
problem
\[
 Q_{rA,rB}(x)=rQ_{A,B}(x^r).
\]
Target (b) remains the cleanest uniform statement.  The new self-power theorem
removes every possible collision having even one irreducible primitive base,
so the unresolved core now lies entirely in the reducible--reducible family.

Write
\[
 P_{a,b}(x)=Q_{a,b}(x)Q_{b,a}(x)
\]
for the reciprocal package product.  Call the unordered pair \(\{a,b\}\)
\emph{globally isolated} if \(P_{a,b}\) is coprime to every
\(P_{c,d}\) with \(\{c,d\}\neq\{a,b\}\).
...
The density estimate uses Borisov's quantitative count of exceptional
primitive shapes.  It does not settle the full conjecture: the remaining
proof requirement is the exclusion of compatible proper factors of two
reducible primitive \(abc\)-polynomials and their compositions.  The
residual-polynomial splitting theorem removes the former local \(2\)-adic
exception, while the height--degree argument and the exact computation in
Section~\ref{sec:computation} eliminate the complete unit-normalized part of
the explicit box.  What remains is the finite nonunit range
\(D\leq6\,816\,241\), for which endpoint-power branching must still be
completed.
...
\texttt{code/certify\_constants.py} uses
\[
...
make test
\end{lstlisting}
The proof pipeline requires a C++20 compiler with OpenMP and Python~3; its
Python verification uses only the standard library.  The exact rational
certificate \texttt{code/certify\_constants.py} prints
...
and Arb script \texttt{code/certify\_thresholds.sage}.  All comparisons in
...
The full conjecture is not yet proved.  The audited upper computation gives
the exact reduction
\[
 D:=\max\{m,n,p,q\}\leq6\,816\,241.
\]
The obstruction to simply extending that computation is that the common
minimal polynomial need not be monic with unit constant term.  Full Kummer
degree and the one-package unit support cap may therefore fail at endpoint
primes.  This section replaces full Kummer degree by exact extraction
degrees and gives a finite state model for every remaining branch.

Write the two normalized packages as
\[
 \{ra,rb\},\qquad \{sc,se\},
 \qquad \gcd(a,b)=\gcd(c,e)=1,
 \qquad \gcd(r,s)=1,
\]
and let \(\alpha\) be a common root of selected orientations.  Put
\[
...
\subsection{Degree bookkeeping without full Kummer degree}
...
\begin{proof}
The element \(\alpha\) satisfies \(X^r-\alpha^r\) over \(K\) and
\(X^s-\alpha^s\) over \(L\), proving the first two inequalities and the
tower identities.  Since \(\gcd(r,s)=1\), an integral B\'ezout combination
of \(r,s\) recovers \(\alpha\) from \(\alpha^r,\alpha^s\), so \(E=KL\).
The compositum inequality
\([KL:\Q]\leq[K:\Q][L:\Q]\) gives the last assertion.
\end{proof}

The last inequality is intentionally weaker than a linear-disjointness
identity.  No such identity is assumed in the lower-range state model.

\subsection{A reduced power-correspondence bound}

\begin{proposition}[General reduced correspondence]
\label{prop:reducedcorrespondence}
Let selected primitive orientations be \(Q_{a,b}(x^r)\) and
\(Q_{c,e}(x^s)\), and suppose they share a noncyclotomic irreducible factor
of degree \(d\).  Put
\[
...
\begin{equation}
 ...
 \tag{19.3}
\end{equation}
with nonzero rational constants.  Their closures in
\(\mathbb P^1\times\mathbb P^1\) have bidegrees
\((E_0,B_0)\) and \((C_0,A_0)\), so their intersection number is
\(E_0A_0+B_0C_0\).

They have no common curve component unless the packages agree.  On the
normalization of a hypothetical component, the two equations give two
linear relations among the four divisors of
\(U_0,1-U_0,V_0,1-V_0\).  Lemma~\ref{lem:fourunit} makes \(V_0\) one of
the six M\"obius transforms of \(U_0\).  In the divisor table from the proof
of Proposition~\ref{prop:sameshapebezout}, positivity in
\[
 E_0\operatorname{div}(U_0)=B_0\operatorname{div}(V_0),\qquad
 C_0\operatorname{div}(1-U_0)=
 A_0\operatorname{div}(1-V_0)
\]
rules out five rows.  The remaining row is \(V_0=U_0\), with
\(E_0=B_0\) and \(C_0=A_0\).  Thus \(se=rb\) and \(sc=ra\);
primitivity gives \((c,e)=(a,b)\) and \(r=s\).

The map \(x\mapsto U_0\) is injective on the relevant roots, because \(U_0\)
determines both \(x^b\) and \(x^{-a}\), and \(\gcd(a,b)=1\).  Similarly
\(V_0\) determines \(y\).  The \(d\) conjugates of \(\alpha\) therefore
give \(d\) distinct intersection points: equality of the two coordinates
recovers \(\alpha^r,\alpha^s\), and then \(\alpha\).
B\'ezout proves \((19.2)\).
\end{proof}

For equal primitive shapes, \(g_0=b\) and \(g_1=a\), so \((19.2)\)
recovers \(d\leq2rs\).

\subsection{Scaled coefficient arithmetic}

Let
\[
 h_\alpha(X)=UX^d+\cdots+V\in\Z[X],\qquad U>0,
\]
be the primitive minimal polynomial of \(\alpha\), and let
\(f_\beta,f_\gamma\) be the primitive minimal polynomials of
\(\beta=\alpha^r,\gamma=\alpha^s\), with leading and constant
coefficients \((u_1,v_1)\), \((u_2,v_2)\).
...
\begin{equation}
 ...
 \tag{19.4}
\end{equation}
In particular, the prime support of each pair \((u_i,v_i)\) equals the
corresponding part of the prime support of \(UV\).
\end{lemma}
...
\]
reads
\((V/U)^r=(v_1/u_1)^t\), up to sign.  Both fractions are in lowest terms,
so numerator and denominator valuations agree prime by prime.  The second
...
\begin{proof}
Apply Proposition~\ref{prop:endpointarith} to \(f_\beta\) and raise its
endpoint divisibilities to the power \(t\).  Lemma~\ref{lem:normtransfer}
and \(te_1=d\) give \((19.5)\).  The second package gives \((19.6)\).
For \((19.7)\), choose a \(p\)-adic embedding at which \(\alpha\) is a
negative-valuation root.  The endpoint Newton polygons give respectively
\(-v_p(a)/(rb)\) and \(-v_p(c)/(se)\).  The positive-valuation argument
gives \((19.8)\).
\end{proof}

\begin{proposition}[Two nonunit coefficients lie in a small box]
\label{prop:twononunit}
If \(U>1\) and \(|V|>1\), then
\begin{equation}
 D\leq37\,799.
 \tag{19.9}
...
\begin{proof}
Choose distinct primes \(p\mid U\), \(q\mid V\); they are distinct because
\(\gcd(U,V)=1\).  Equation \((19.7)\) implies
\[
 B_0\leq v_p(a),\qquad E_0\leq v_p(c),
\]
and \((19.8)\) gives
\[
 A_0\leq v_q(b),\qquad C_0\leq v_q(e).
\]
Proposition~\ref{prop:reducedcorrespondence} therefore yields
\[
 d\leq v_p(c)v_q(b)+v_p(a)v_q(e)
 \leq2\lfloor\log_2D\rfloor\lfloor\log_3D\rfloor.
\]
Together with \(d>D/140\), this is impossible for \(D\geq37\,800\).
The exact integer transition is certified by
\texttt{code/lower/certify\_lower\_constants.py}.
\end{proof}

\begin{corollary}[Coefficient trichotomy above \(50\,000\)]
\label{cor:coefftrichotomy}
If \(50\,000\leq D\leq6\,816\,241\), then
\[
 M(h_\alpha)<13.
\]
Consequently either \((U,|V|)=(1,1)\), or exactly one of \(U,|V|\) is a
nonunit integer in \(\{2,3,\ldots,12\}\).
\end{corollary}

\begin{proof}
The height--degree estimate used in Section~\ref{sec:computation} gives
\[
 \log M(h_\alpha)<\frac{8\log(2D)}{D^{1/3}}.
\]
The right side is decreasing for \(D\geq50\,000\).  The exact rational
interval certificate accompanying this revision proves that its value at
\(50\,000\) is below \(\log13\).  The leading and constant coefficients of
a primitive polynomial are at most its Mahler measure.
Proposition~\ref{prop:twononunit} excludes two nonunit coefficients in this
range.
\end{proof}

\subsection{Deficient scale primes}
...
The following alternatives exhaust the resulting local branch.
\begin{enumerate}[label=(\alph*)]
\item If \(p\) is good for the primitive shape, then
\begin{equation}
 e_1\leq p-2.
 \tag{19.10}
\end{equation}
\item A total-role prime is excluded by
Corollary~\ref{cor:totalprimepower}.
\item An endpoint-role prime is excluded when \(f_\beta\) is a unit
polynomial.  In the remaining nonunit endpoint packet, writing
\(p^\kappa\Vert a\) when the leading coefficient is nonunit, one has
\begin{equation}
 e_1\leq b,\qquad pb\mid e_1\kappa.
 \tag{19.11}
\end{equation}
The reciprocal statement holds when the constant coefficient is nonunit.
Moreover \(p\in\{2,3,5,7\}\).
\end{enumerate}
In particular every unit Kummer-defect counterexample satisfies
\begin{equation}
...
\begin{proof}
If \(\beta\notin K^p\) and the exceptional fourth-power condition does not
occur, Capelli's criterion makes \(X^{p^a}-\beta\) irreducible.  The field
\(K(\alpha^{r/p^a})\) is then a subfield of \(E\) of degree \(p^a\), so
\(p^a\mid t\), a contradiction.

The good-prime alternative is Theorem~\ref{thm:frobeniusfactor}; the
total-role exclusion is Corollary~\ref{cor:totalprimepower}.  At an endpoint,
Theorem~\ref{thm:endpointpower} excludes a unit factor and gives
\((19.11)\) for the nonunit packet, including the exceptional fourth-power
branch at \(p=2\).  Its necessary condition \(\kappa\geq p\), together with
\(p^\kappa\leq D\leq6\,816\,241\), leaves
\(p=2,3,5,7\).

For a unit factor every deficient prime is therefore good and satisfies
\(e_1\leq p-2\).  Since \(d=te_1\leq re_1\), \(r\leq139\), and
\(p\leq139\), the inequality \(d>D/140\) gives \((19.12)\).
\end{proof}

\subsection{A finite certificate object}

\begin{definition}[Lower endpoint state]\label{def:endpointstate}
A one-package state is a tuple
\[
 (r,t,e_1,d,U,V;a,b,\varepsilon),
\]
where \(\varepsilon\) records the selected orientation.  A two-package state
consists of two such tuples with a common \((d,U,V)\).  Put
\[
 N=a+b,\qquad M=c+e,\qquad D=\max(rN,sM).
\]
It is \emph{admissible} if all of the following hold:
\begin{enumerate}[label=(\roman*)]
\item \(1\leq t\leq r\leq139\), \(d=te_1\),
\(2\leq e_1\leq N-4\), and analogously on the second side;
\item \(140d>D\), \(d^3<64D^2\), and
\(d\leq\min(N,M)-2\);
\item for every coefficient prime \(\ell\),
\[
 t\mid r v_\ell(U),\quad t\mid r v_\ell(V),
\]
and analogously for \((s,u,e_2)\);
\item the scaled endpoint divisibilities \((19.5)\)--\((19.6)\), the slope
equalities \((19.7)\)--\((19.8)\), and the correspondence bound \((19.2)\);
\item every prime in the primitive additive triple outside
\(\operatorname{Supp}(UV)\) satisfies the factor-degree alternatives of
Theorem~\ref{thm:radicalsieve}, Proposition~\ref{prop:rolesignature}, and
Corollary~\ref{cor:partitionsignature};
\item every deficient scale prime satisfies one of the three alternatives in
Proposition~\ref{prop:defectlocalization};
\item the scaled additive triples are disjoint, the scales are coprime, and
the two packages are distinct.
\end{enumerate}
\end{definition}

\begin{theorem}[Finite lower-state envelope]
\label{thm:lowerstatereduction}
Every normalized counterexample with \(D\leq6\,816\,241\) determines an
admissible lower endpoint state.  The set of such states is finite and
effectively enumerable.  Consequently, an exhaustive elimination of the
admissible states proves the conjecture.
\end{theorem}

\begin{proof}
Lemmas~\ref{lem:extractiondegrees} and \ref{lem:normtransfer},
Propositions~\ref{prop:reducedcorrespondence},
\ref{prop:scaledendpoint}, and \ref{prop:defectlocalization}, and the exact
local factor signatures of Section~\ref{sec:degreesieve} give the listed
necessary conditions.  All integer entries are explicitly bounded by
\(D\leq6\,816\,241\), \(r,s\leq139\), the degree bounds, and the endpoint
divisibilities.  Thus the state set is finite and can be enumerated with
integer arithmetic.  The last sentence is the contrapositive of the first.
\end{proof}

\subsection{Computational implementation and present assurance boundary}

The accompanying bundle separates four layers:
\begin{enumerate}[label=(\arabic*)]
\item the independently audited upper-range program and deterministic data;
\item exact lower constants, a machine-readable endpoint-state schema, and
a reference checker which recomputes all global arithmetic constraints;
\item a literal small-domain endpoint-state enumerator, used for coverage
regressions independently of a future optimized production generator;
\item terminal finite-field certificates.  A certificate records a good
auxiliary prime for which the two collision trinomials preserve degree and
have modular gcd of degree exactly two; a separate checker recomputes that
claim.
\end{enumerate}

Development runs found no terminal survivor in the implemented branches and
indicate that arithmetic complexity is comfortably below the stated
25-core, seven-day budget.  Those observations are not promoted to a theorem
in this revision.  The remaining assurance obligation is to consolidate the
development enumerators into one deterministic production generator, emit a
branch-coverage manifest, and compare it with a structurally independent
coverage implementation.  The exact checklist is in
\texttt{docs/lower/PROOF\_BOUNDARY.md}.

\begin{conjecture}[Residual four-index lemma]\label{conj:fourindex}
For \(|z|\neq1\),
\[
 L_m(z)=L_n(z),\qquad L_p(z)=L_q(z),
 \qquad m<n,\ p<q,
\]
implies \(\{m,n\}=\{p,q\}\).
\end{conjecture}
...
The audited upper computation excludes
\[
 6\,816\,242\leq D\leq175\,394\,637.
\]
The present revision supplies the mathematical architecture needed below
that range without assuming full Kummer degree.  It proves the general
reduced-correspondence bound, transfers endpoint coefficients through exact
norm identities, confines the two-nonunit branch to \(D\leq37\,799\),
confines every unit Kummer-defect branch to \(D<2\,666\,020\), and reduces
the remaining nonunit defects to endpoint packets at
\(p\in\{2,3,5,7\}\).

This is a finite and realistically sized computer-assisted proof program,
but the package does not yet declare the conjecture proved.  Its outstanding
obligation is now explicit and software-theoretic: a production lower-state
manifest and an independent coverage audit, followed by verification of the
terminal modular certificates.  Keeping that boundary visible is essential
