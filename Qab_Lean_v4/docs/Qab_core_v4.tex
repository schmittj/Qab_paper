...
The first concrete replacement for opaque \verb|PackageShare| must be careful
about primitive versus imprimitive pairs.  The coefficient formula below is for
the primitive orientation polynomial only.  For coprime positive $A,B$, define
\[
Q^{\rm prim}_{A,B}(x)=
